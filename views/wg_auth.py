...
def wg_auth():
    ...
    if discord_id_str is None:
        ...
        identities = asyncio.run_coroutine_threadsafe(verify.verify(), bot_obj.loop).result()  # noqa
        if not identities:
...
def comp_auth(discord_id: int, account_id: str, nickname: str) -> str:
    ...
    if region == "ERROR":
        ...
        asyncio.run_coroutine_threadsafe(add_role_authed(bot_obj, discord_id), bot_obj.loop)  # noqa
        # DBへの情報の登録
        asyncio.run_coroutine_threadsafe(db.add_user(discord_id, account_id, region), bot_obj.loop)  # noqa
...
def callback(url):
    ...
    if access_token is None:
        raise FlaskCustomError("エラー", ["エラーが発生しました。", "お手数ですが再度Discordからお試しください。"], "E20001")
