...
class Verification:
    ...
    async def is_positive_assertion(self):
        """Positive Assertions
    ...
    async def verify_return_url(self):
        ...
        """
        return_to = await self.return_to
        #
        if self.assertion.scheme != return_to.scheme or \
                self.assertion.netloc != return_to.netloc or \
                self.assertion.path != return_to.path:
            reason = 'scheme/authority/path are not the same'
            try:
                raise OpenIDFailReturnURLVerification(reason)
            except OpenIDFailReturnURLVerification as e:
                logger.error(e)

        query_parameters = parse_qs(return_to.query)
    ...
    async def verify_discovered_information(self):
    ...
    async def check_nonce(self):
        ...
        if not await self.reader(nonce):
            await self.saver(nonce)
            return True
        else:
            return False

    async def verify_signatures(self):
        ...
        request = post(await self.op_endopint, to_sign, allow_redirects=False,
                       headers=headers)

        # Verification parsing
        verification = await self.parse_l2l(request.text)

        return verification.get('is_valid', False)

    async def identify_the_end_user(self):
    ...
    async def parse_l2l(self, response):
    ...
    async def verify(self):
        ...
        for validator in validators:
            ...
            if not is_valid:
                ...
                except OpenIDVerificationFailed as e:
                    logger.error(e)
                    return False

        return await self.identify_the_end_user()

    @property
    async def op_endopint(self):
        query = parse_qs(self.assertion.query)
